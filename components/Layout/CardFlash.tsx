...
const CardFlash = ({
  ...
  return (
    <Card className={cn('w-full max-w-5xl mx-auto', className)}>
      <CardHeader>
        <CardHeading>
          <CardTitle className="text-lg text-muted-foreground text-white">
            {subtitle}
          </CardTitle>
        </CardHeading>
      </CardHeader>
        ...
        >
          <div className="flex items-center gap-4">
            {icon && (
              <div className="flex items-center justify-center rounded-full border border-gray-300 w-9 h-9">
                <i
                  className={`ki-filled ${icon} text-base`}
                  aria-hidden="true"
                ></i>
              </div>
            )}
          ...
          {description && Array.isArray(description) ? (
            description.map((paragraph, index) => (
              <p
                key={index}
                className="text-sm text-muted-foreground text-justify leading-relaxed"
              >
                {paragraph}
              </p>
            ))

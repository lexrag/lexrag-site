...
export const badgeColorMapping: Record<string, 'primary' | 'success' | 'warning' | 'info' | 'destructive' | 'secondary'> = {
  search: 'primary',
  query: 'success',
  storage: 'warning',
  analytics: 'info',
...
export const getBadgeColor = (badge: string): 'primary' | 'success' | 'warning' | 'info' | 'destructive' | 'secondary' => {
  return badgeColorMapping[badge] ?? 'secondary';
};

// 2. Maps category values (e.g. "search") to icon/border/gradient colors for cards
export const categoryColorScheme: Record<string, {
  icon_color: string;
  border: string;
  gradient: string;
}> = {
...
export const getCategoryColorScheme = (category: string): {
  icon_color: string;
  border: string;
  gradient: string;
} => {
  return categoryColorScheme[category] ?? {

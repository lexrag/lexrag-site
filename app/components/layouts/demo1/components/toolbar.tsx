...
function ToolbarHeading({ title = '', description }: ToolbarHeadingProps) {
    ...
    return (
        <div className="flex flex-col justify-center gap-2">
            <h1 className="text-xl font-medium leading-none text-mono">{title || item?.title || 'Untitled'}</h1>
            {description && (
                <div className="flex items-center gap-2 text-sm font-normal text-muted-foreground">{description}</div>
            )}
        </div>
    );
}
